...
const ResumeAnalysisResultsPage = () => {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Top Header & Actions */}
      <div className="glass-panel p-6 sm:p-8 rounded-3xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-xs text-slate-400">Generated on {new Date(data.created_at).toLocaleDateString()}</p>
        </div>

        <button
          onClick={handleDownloadPDF}
          disabled={downloading}
          className="px-6 py-3 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs shadow-lg shadow-indigo-600/30 transition-all flex items-center space-x-2 disabled:opacity-50"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {data.job_recommendations.map((job, idx) => (
            <div key={idx} className="glass-card p-4 rounded-2xl flex items-center justify-between">
              ...
              <span className="px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-400 font-extrabold text-xs shrink-0">
